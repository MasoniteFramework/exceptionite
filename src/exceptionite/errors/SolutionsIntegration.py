...
class GetAttributeController:
    ...
    def description(self):
        return "This error is because of either a syntax error in your controller or an invalid import at the top of your controller."
...
class CSRFIssue:
    ...
    def description(self):
        return "CSRF tokens are used to make sure nobody is making requests to your web app from the outside. If you just submitted a form request make sure you put the {{ csrf_field }} inside the form tag"
...
class SolutionsIntegration:
    ...
    def content(self, handler):
        ...
        loader = DictLoader({"solutions.html": overflow_exception, })

        environment = Environment(
            loader=loader, autoescape=select_autoescape(["html", "xml"])
        )
        return environment.get_template("solutions.html").render(
